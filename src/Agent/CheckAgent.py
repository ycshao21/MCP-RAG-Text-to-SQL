...
def parse_response(response: str):
    ...
    Check_completed = Check_completed_match.group(1) == "True" if Check_completed_match else False
...
class CheckAgent(BaseClient):
    async def run(self, description: str, sql: str):
        ...
        If the original SQL is incorrect, and you come up with a correct SQL, please write the correct SQL in the adjustment.
        ...
        print(f"Initial result: {result}, Match: {is_match}, Check completed: {check_completed}, Adjustment: {adjustment}")
        ...
        while not check_completed and attempt < max_attempts:
            ...
            response = await self.process_query(adjusted_query, system_prompt=system_prompt)
            result, is_match, check_completed, adjustment = parse_response(response)
            ...
            attempt += 1
            print(f"Attempt {attempt}: Result: {result}, Match: {is_match}, Check completed: {check_completed}, Adjustment: {adjustment}")
